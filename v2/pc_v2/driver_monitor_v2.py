...
from .serial_protocol import parse_line, format_cmd_mode
...
def main():
    ...
    consec = 0; autopilot = False; last_open = time.time()
    while True:
        ok, frame = cap.read()
        if not ok:
        ...
        if res.multi_face_landmarks:
            ...
            try:
                L = ear4(p(159),p(145),p(33),p(133)); R = ear4(p(386),p(374),p(263),p(362))
                ear = float((L+R)/2.0)
